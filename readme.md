...
3. Drop the ranger.ks folder into the kOS scripts directory (usually located at `Kerbal Space Program/Ships/Script`).

### Usage:

1. Create a new kOS script that copies the RANGER script to the CPU's local volume:
```
copypath("0:/[your ranger folder]/ranger.ks", "1:/").
```
...
3. Call the RANGER update function in your main control loop. Include an exit condition to break out of the loop once your spaceplane has transitioned to normal atmospheric flight (I find 1200m/s a decent cutoff point). Also set a steering variable prior to the loop, and LOCK STEERING to it. Be sure to unlock the steering afterwards.


```
local reentrySteering to ship:facing. //temporary direction just to initialize the variable
...
4. Using Trajectories, plan and execute a deorbit burn that targets a landing site of your choice. I recommend setting your reentry parameters in Trajectories to maintain a specific (20-40 degree) angle of attack during the Entry and High Altitude phases, and zero during the Low Altitude and Final Approach phases.

5. Call your reentry script after the deorbit burn is complete. RANGER will maintain surface prograde until entry interface, and then begin providing AoA and bank angle guidance once in the atmosphere.
    
RANGER will discontinue providing active guidance if the ABORT action is triggered, however your loop should also take the ABORT action into account as an exit condition. You can add another condition to your UNTIL loop to break free if this happens:
```
